...
{ QMSG Message;
  ...
  if((Argv[1].strptr==NULL || Argv[1].strlength==0) || (Argv[2].strptr==NULL || Argv[2].strlength==0))
    { /* winAttrs|=FCF_SHELLPOSITION; */ /* что это за бред  ? */
      xOrigin=1;
      yOrigin=1;    
    }
   else
  ...
  /* вот тут и была бага. Часто очередь в pmrexx уже создана. 
  Причём очень странная ;) */  
  if(!hmqMain) 
 ...
 /* для того, чтобы кнопка нажатая для вызова DLL не сработала */  
  while(WinGetMsg(habMain, &Message, (HWND)NULL, 0, 0 ))
   { if(Message.msg==WM_CHAR)
       { if((scancode==-1) && (SHORT1FROMMP(Message.mp1) & KC_KEYUP) && (!(SHORT1FROMMP(Message.mp1) & KC_DEADKEY)) && (SHORT1FROMMP(Message.mp1) & KC_LONEKEY))
